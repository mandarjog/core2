...
import (
	...
	"istio.io/core/tools/codegen/pkg/bootstrapgen"
)
...
func withArgs(args []string, errorf func(format string, a ...interface{})) {
	...
	rootCmd := cobra.Command{
		...
		Long: "Each of the input <File descriptor set protobuf> must contain a proto file that defines the template.\n" +
			...
			"Example: " +
			"mixgenbootstrap metricTemplateFileDescriptorSet.pb:istio.io/core/template/metric " +
			"quotaTemplateFileDescriptorSet.pb:istio.io/core/template/quota " +
			"listTemplateFileDescriptorSet.pb:istio.io/core/template/list " +
			"-o template.gen.go",
			...
			for _, arg := range args {
				...
				if len(m) != 2 {
					errorf("Invalid argument '%s'. Argument should contain one colon and be of the form <Path To File DescriptorSet "+
						...
						"Example: mixgenbootstrap metricTemplateFileDescriptorSet.pb:istio.io/core/template/metric", arg)
				}
