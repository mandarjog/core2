...
MIXER_DEPS = [
    "@io_istio_core//pkg/adapter:go_default_library",
...
def _gen_template_and_handler(name, importmap = {}):
   ...
   genrule_args = {
       ...
       "tools": [ "@io_istio_core//tools/codegen/cmd/mixgenproc" ],
       "message": "Generating handler code from descriptor",
       "cmd": "$(location @io_istio_core//tools/codegen/cmd/mixgenproc) "
...
def _mixer_supported_template_gen(name, packages, out):
  ...
  native.genrule(
      ...
      srcs = descriptors,
      outs = [out],
      cmd = "$(location @io_istio_core//tools/codegen/cmd/mixgenbootstrap) " + args + " -o $(location %s)" % (out),
      tools = ["@io_istio_core//tools/codegen/cmd/mixgenbootstrap"],
  )

DEPS_FOR_ALL_TMPLS = [
    "@io_istio_core//pkg/adapter:go_default_library",
    "@io_istio_core//pkg/attribute:go_default_library",
    "@io_istio_core//pkg/expr:go_default_library",
    "@io_istio_core//pkg/template:go_default_library",
    "@com_github_gogo_protobuf//proto:go_default_library",
